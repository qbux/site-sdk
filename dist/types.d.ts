...
export interface SiteHttpClient {
    ...
    post<T = unknown>(url: string, body: unknown, options?: SiteHttpRequestOptions): Promise<SiteHttpResponse<T>>;
}
...
export type LoginError = 'INVALID_INPUT' | 'LOGIN_FAILED' | 'CAPTCHA_FAILED' | 'REQUEST_FAILED';
...
export interface SiteFailureClassification {
    retryable: boolean;
    reason: 'SESSION_EXPIRED' | 'RATE_LIMITED' | 'NOT_FOUND' | 'NO_RESOURCE' | 'REQUEST_FAILED';
}
